...
static const struct flash_pages_layout ospi_b_flash_ra_layout[3] = {
	{.pages_count = 32, .pages_size = SECTOR_SIZE_4K},
	{.pages_count = 1, .pages_size = SECTOR_SIZE_128K},
	{.pages_count = 255, .pages_size = SECTOR_SIZE_256K}};
					     ...
					     size_t len)
{
	...
	if (ospi_b_data->ospi_b_ctrl.spi_protocol == SPI_FLASH_PROTOCOL_EXTENDED_SPI) {
		direct_transfer[TRANSFER_READ_SFDP_ID_SPI].address = offset;
		transfer = direct_transfer[TRANSFER_READ_SFDP_ID_SPI];

	} else {
		direct_transfer[TRANSFER_READ_SFDP_ID_OSPI].address = offset;
